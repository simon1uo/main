import { metricsName, PageInformation, routeType, userAgent } from '@types'
...
export const getClickInform = function (e: MouseEvent | any): void {
  ...
  let target = e.path?.find((x: Element) =>
    clickMountList.includes(x.tagName?.toLowerCase())
  )
  // 不支持 path 就再判断 target
  target =
    target ||
    (clickMountList.includes(e.target.tagName?.toLowerCase())
      ? e.target
      : undefined)
  if (!target) return
...
export function historyPageTrack(): void {
  ...
  function getDuration(): { curTime: number; duration: number } {
    ...
    const duration = curTime - beforeTime
    beforeTime = curTime
...
export function hashPageTrack(): void {
  ...
  function getDuration(): { curTime: number; duration: number } {
    ...
    const duration: number = curTime - beforeTime
    beforeTime = curTime
    return { curTime, duration }
  }
