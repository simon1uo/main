import { ConsoleError } from '@monitor/class'
import { lazyReport } from '@monitor/report'
import { BlackScreenDataType, httpMetrics, mechanismType } from '@types'
import {
  calcDuration,
  createJsError,
  createResourceError,
...
import { isLoad } from '@utils/common'
...
export function handleJs(event: any): void {
  ...
  if (type === mechanismType.RS) {
    ...
    const { url, method, params, data } = event.config
    const { name, message, response, request } = event

    const corsErrorData = createCorsError(
      name,
      message,
      url,
      method,
...
export const handlePromise = function (event: any): void {
  ...
  if (!isCors) {
    ...
    const selector = lastEvent ? getSelector((lastEvent as any).path) : ''
    const promiseErrorData = createPromiseError(
      message,
      event,
      filename,
      line,
...
export const injectConsole = () => {
  ...
  window.console.error = function (error) {
    if (error != '参数有缺失') {
      ...
      const consoleErrorData = createConsoleError(
        message,
        column,
        row,
        stack,
      ...
      if (stack) {
        const mres = stack.match(/\(.*?\)/g) || []
        const firstLine = (mres[0] || '').replace('(', '').replace(')', '') // 获取到堆栈信息的第一条
...
const proxyXmlHttp = (
  ...
  function isExistsXMLHttpRequest() {
    return (
      'XMLHttpRequest' in window && typeof window.XMLHttpRequest === 'function'
    )
  }
}
...
export const blankScreen = (): void => {
  ...
  // 屏幕上空白点的数量
  let { wrapperElements, emptyPoints }: BlackScreenDataType =
    initElementsAndPoints()
  const getSelector = function (element) {
    ...
    } else return nodeName ? nodeName.toLowerCase() : element
  ...
  const judge = function (): void {
    ...
    if (emptyPoints >= 18) {
      const centerElements: Element[] = document.elementsFromPoint(
        innerWidth / 2,
        innerHeight / 2
      )
      const log = {
  ...
  function initElementsAndPoints() {
    const wrapperElements: string[] = [
      'html',
      'body',
      '#container',
      '.content.main'
    ]
    const emptyPoints = 0 // 屏幕上空白点的数量
